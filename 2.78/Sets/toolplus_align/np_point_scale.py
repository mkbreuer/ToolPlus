...
class NPPSGetContext(bpy.types.Operator):
    bl_idname = 'object.np_ps_get_context_np'
...
class NPPSGetSelection(bpy.types.Operator):
    bl_idname = 'object.np_ps_get_selection_np'
...
class NPPSPrepareContext(bpy.types.Operator):
    bl_idname = 'object.np_ps_prepare_context_np'
    ...
    def execute(self, context):
        ...
        print ('prepare, NP020PS.flag = ', flag)
...
class NPPSDisplayCage(bpy.types.Operator):
    bl_idname = 'object.np_ps_display_cage_np'
    ...
    def modal(self, context, event):
        ...
        if event.type in ('LEFTMOUSE', 'RIGHTMOUSE', 'RET', 'NUMPAD_ENTER'):
...
class NPPSRunResize(bpy.types.Operator):
    bl_idname = 'object.np_ps_run_resize_np'
    ...
    def invoke(self, context, event):
        ...
        if context.area.type == 'VIEW_3D':
            if flag == 'EXIT':
                print('RunResize_INVOKE_DECLINED_FINISHED',';','flag = ', flag)
                return {'FINISHED'}
            elif flag == 'RUNRESIZE':
                axis = NP020PS.axis
            args = (self, context)
            ...
            print('RunResize_INVOKE_a_RUNNING_MODAL')
            return {'RUNNING_MODAL'}
        else:
            self.report({'WARNING'}, "View3D not found, cannot run operator")
            print('RunResize_INVOKE_DECLINED_FINISHED',';','flag = ', Storage.flag)
            return {'FINISHED'}
...
class NPPSRestoreContext(bpy.types.Operator):
    bl_idname = "object.np_ps_restore_context_np"
