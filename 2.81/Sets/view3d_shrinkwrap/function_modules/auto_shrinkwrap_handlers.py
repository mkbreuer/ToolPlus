...
def save_pre(scene):
    # Disable automatic shrinkwrapping before saving.
    props = addon.preferences.surface_constraint
    props.auto_shrinkwrap_is_enabled_pre_save = props.auto_shrinkwrap_is_enabled
    if props.auto_shrinkwrap_is_enabled_pre_save:
        props.auto_shrinkwrap_is_enabled = False
...
def scene_update_pre(scene):
    ...
    if (
        ...
        if (
            ...
            modifier = active_object.modifiers.new(name = "Surface Constraint Preview", type = 'SHRINKWRAP')
        ...
        # Otherwise, validate the existing, referenced modifier.
            ...
            if modifier.target != bpy.data.objects[target]:
                modifier.target = bpy.data.objects[target]          
            if modifier.wrap_mode != 'ABOVE_SURFACE':
                modifier.wrap_mode = 'ABOVE_SURFACE'       
        ...
        # has finished.
        if (active_operator and str(active_operator.as_pointer()) != props.operator_uid):
            # Apply the modifier.
