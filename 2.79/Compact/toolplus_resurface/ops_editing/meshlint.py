...
try:
    ...
    class MeshLintControl(bpy.types.Panel):
        ...
        @classmethod
        def poll(cls, context):
            return has_active_mesh(context) and context.mode == 'EDIT_MESH'
        ...
        def add_toggle_buttons(self, layout, context):
            ...
            if tp_props.display_meshlint_toggle:
                row.prop(tp_props, "display_meshlint_toggle", text="Toggle Options", icon='TRIA_DOWN')
            else:
                row.prop(tp_props, "display_meshlint_toggle", text="Toggle Options", icon='TRIA_RIGHT')
