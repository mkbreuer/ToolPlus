...
def cache_delete(tool):
    if tool in tp_looptools_cache:
        del tp_looptools_cache[tool]


# check cache for stored information
def cache_read(tool, object, bm, input_method, boundaries):
    # current tool not cached yet
    if tool not in tp_looptools_cache:
        return(False, False, False, False, False)
    # check if selected object didn't change
    if object.name != tp_looptools_cache[tool]["object"]:
        return(False, False, False, False, False)
    # check if input didn't change
    if input_method != tp_looptools_cache[tool]["input_method"]:
        return(False, False, False, False, False)
    if boundaries != tp_looptools_cache[tool]["boundaries"]:
        return(False, False, False, False, False)
    modifiers = [mod.name for mod in object.modifiers if mod.show_viewport and
                 mod.type == 'MIRROR']
    if modifiers != tp_looptools_cache[tool]["modifiers"]:
        return(False, False, False, False, False)
    input = [v.index for v in bm.verts if v.select and not v.hide]
    if input != tp_looptools_cache[tool]["input"]:
        return(False, False, False, False, False)
    # reading values
    single_loops = tp_looptools_cache[tool]["single_loops"]
    loops = tp_looptools_cache[tool]["loops"]
    derived = tp_looptools_cache[tool]["derived"]
    mapping = tp_looptools_cache[tool]["mapping"]
...
loops, derived, mapping):
    ...
    modifiers = [mod.name for mod in object.modifiers if mod.show_viewport and
                 mod.type == 'MIRROR']
    # update cache
    tp_looptools_cache[tool] = {"input": input, "object": object.name,
...
def gstretch_update_max(self, context):
    ...
    # called from toolbar
        ...
        tp = context.window_manager.tp_looptools
...
class Bridge(Operator):
    bl_idname = 'mesh.tp_looptools_bridge'
    ...
    def invoke(self, context, event):
        ...
        context.window_manager.tp_looptools.bridge_loft = self.loft
        settings_load(self)
...
class Circle(Operator):
    bl_idname = "mesh.tp_looptools_circle"
    bl_label = "Circle"
...
class Curve(Operator):
    bl_idname = "mesh.tp_looptools_curve"
    bl_label = "Curve"
...
class Flatten(Operator):
    bl_idname = "mesh.tp_looptools_flatten"
    bl_label = "Flatten"
...
class GStretch(Operator):
    bl_idname = "mesh.tp_looptools_gstretch"
    bl_label = "Gstretch"
    ...
    def invoke(self, context, event):
        ...
        if 'Gstretch' in tp_looptools_cache:
            tp_looptools_cache['Gstretch']['single_loops'] = []
        # load custom settings
        settings_load(self)
        return self.execute(context)
...
class Relax(Operator):
    bl_idname = "mesh.tp_looptools_relax"
    bl_label = "Relax"
...
class Space(Operator):
    bl_idname = "mesh.tp_looptools_space"
    bl_label = "Space"
...
#class VIEW3D_MT_edit_mesh_tp_looptools(Menu):
#    bl_label = "tp_looptools"

#    def draw(self, context):
#        layout = self.layout

#        layout.operator("mesh.tp_looptools_bridge", text="Bridge").loft = False
#        layout.operator("mesh.tp_looptools_circle")
#        layout.operator("mesh.tp_looptools_curve")
#        layout.operator("mesh.tp_looptools_flatten")
#        layout.operator("mesh.tp_looptools_gstretch")
#        layout.operator("mesh.tp_looptools_bridge", text="Loft").loft = True
#        layout.operator("mesh.tp_looptools_relax")
#        layout.operator("mesh.tp_looptools_space")


## panel containing all tools
#class VIEW3D_PT_tools_tp_looptools(Panel):
...
#        if tp.display_bridge:
#            split.prop(tp, "display_bridge", text="", icon='DOWNARROW_HLT')
#        else:
#            split.prop(tp, "display_bridge", text="", icon='RIGHTARROW')
#        split.operator("mesh.tp_looptools_bridge", text="Bridge").loft = False
#        # bridge - settings
#        if tp.display_bridge:
...
#            col_left = row.column(align=True)
#            col_right = row.column(align=True)
#            col_right.active = tp.bridge_segments != 1
#            col_left.prop(tp, "bridge_segments")
#            col_right.prop(tp, "bridge_min_width", text="")
#            # bottom row
#            bottom_left = col_left.row()
#            bottom_left.active = tp.bridge_segments != 1
#            bottom_left.prop(tp, "bridge_interpolation", text="")
#            bottom_right = col_right.row()
#            bottom_right.active = tp.bridge_interpolation == 'cubic'
#            bottom_right.prop(tp, "bridge_cubic_strength")
#            # boolean properties
#            col_top.prop(tp, "bridge_remove_faces")

#            # override properties
#            col_top.separator()
#            row = box.row(align=True)
#            row.prop(tp, "bridge_twist")
#            row.prop(tp, "bridge_reverse")

#        # circle - first line
#        split = col.split(percentage=0.15, align=True)
#        if tp.display_circle:
#            split.prop(tp, "display_circle", text="", icon='DOWNARROW_HLT')
#        else:
#            split.prop(tp, "display_circle", text="", icon='RIGHTARROW')
#        split.operator("mesh.tp_looptools_circle")
#        # circle - settings
#        if tp.display_circle:
#            box = col.column(align=True).box().column()
#            box.prop(tp, "circle_fit")
#            box.separator()

#            box.prop(tp, "circle_flatten")
#            row = box.row(align=True)
...
#        split = col.split(percentage=0.15, align=True)
#        if tp.display_curve:
#            split.prop(tp, "display_curve", text="", icon='DOWNARROW_HLT')
#        else:
#            split.prop(tp, "display_curve", text="", icon='RIGHTARROW')
#        split.operator("mesh.tp_looptools_curve")
#        # curve - settings
#        if tp.display_curve:
#            box = col.column(align=True).box().column()
#            box.prop(tp, "curve_interpolation")
#            box.prop(tp, "curve_restriction")
#            box.prop(tp, "curve_boundaries")
#            box.prop(tp, "curve_regular")
#            box.separator()
...
#            split.prop(tp, "display_flatten", text="", icon='DOWNARROW_HLT')
#        else:
#            split.prop(tp, "display_flatten", text="", icon='RIGHTARROW')
#        split.operator("mesh.tp_looptools_flatten")
#        # flatten - settings
#        if tp.display_flatten:
#            box = col.column(align=True).box().column()
#            box.prop(tp, "flatten_plane")
#            # box.prop(tp, "flatten_restriction")
#            box.separator()
...
#        # gstretch - first line
#        split = col.split(percentage=0.15, align=True)
#        if tp.display_gstretch:
#            split.prop(tp, "display_gstretch", text="", icon='DOWNARROW_HLT')
#        else:
#            split.prop(tp, "display_gstretch", text="", icon='RIGHTARROW')
#        split.operator("mesh.tp_looptools_gstretch")
#        # gstretch settings
#        if tp.display_gstretch:
#            box = col.column(align=True).box().column()
#            box.prop(tp, "gstretch_method")
...
#        split = col.split(percentage=0.15, align=True)
#        if tp.display_loft:
#            split.prop(tp, "display_loft", text="", icon='DOWNARROW_HLT')
#        else:
#            split.prop(tp, "display_loft", text="", icon='RIGHTARROW')
#        split.operator("mesh.tp_looptools_bridge", text="Loft").loft = True
#        # loft - settings
#        if tp.display_loft:
#            box = col.column(align=True).box().column()
#            # box.prop(self, "mode")
...
#            # boolean properties
#            col_top.prop(tp, "bridge_remove_faces")
#            col_top.prop(tp, "bridge_loft_loop")
...
#        split = col.split(percentage=0.15, align=True)
#        if tp.display_relax:
#            split.prop(tp, "display_relax", text="", icon='DOWNARROW_HLT')
#        else:
#            split.prop(tp, "display_relax", text="", icon='RIGHTARROW')
#        split.operator("mesh.tp_looptools_relax")
#        # relax - settings
#        if tp.display_relax:
#            box = col.column(align=True).box().column()
#            box.prop(tp, "relax_interpolation")
#            box.prop(tp, "relax_input")
#            box.prop(tp, "relax_iterations")
#            box.prop(tp, "relax_regular")

#        # space - first line
#        split = col.split(percentage=0.15, align=True)
#        if tp.display_space:
#            split.prop(tp, "display_space", text="", icon='DOWNARROW_HLT')
#        else:
#            split.prop(tp, "display_space", text="", icon='RIGHTARROW')
#        split.operator("mesh.tp_looptools_space")
#        # space - settings
#        if tp.display_space:
#            box = col.column(align=True).box().column()
#            box.prop(tp, "space_interpolation")
#            box.prop(tp, "space_input")
#            box.separator()
...
class tp_looptoolsProps(PropertyGroup):
    """
    Fake module like class
    bpy.context.window_manager.tp_looptools
    """
...
#    self.layout.menu("VIEW3D_MT_edit_mesh_tp_looptools")
...
#panels = (
#        VIEW3D_PT_tools_tp_looptools,
#        )


#def update_panel(self, context):
#    message = "tp_looptools: Updating Panel locations has failed"
#    try:
#        for panel in panels:
...
classes = [
#    VIEW3D_MT_edit_mesh_tp_looptools,
#    VIEW3D_PT_tools_tp_looptools,
    tp_looptoolsProps,
    Bridge,
    Circle,
...
#    bpy.types.VIEW3D_MT_edit_mesh_specials.prepend(menu_func)
    bpy.types.WindowManager.tp_looptools = PointerProperty(type=tp_looptoolsProps)
...
#    bpy.types.VIEW3D_MT_edit_mesh_specials.remove(menu_func)
    try:
        del bpy.types.WindowManager.tp_looptools
    except:
        pass
