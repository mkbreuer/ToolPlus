...
# VERTEX GROUP A #
def create_vertex_groups_endbow_A(self, context):
    ...
    sel_id_A = bpy.context.object.vertex_groups.active_index  
...
# VERTEX GROUP B #
def create_vertex_groups_endbow_B(self, context):
    ...
    sel = bpy.context.object.vertex_groups.active.name
    sel_id_B = bpy.context.object.vertex_groups.active_index  
...
class VIEW3D_TP_EndBow(bpy.types.Operator):
    ...
    # BOW #
    ebb_bow_pin_use = bpy.props.BoolProperty(name="Pin", description="get all except border", default=False)
    ...
    def execute(self, context):    
        ...
        create_vertex_groups_endbow_A(self, context)
        bpy.ops.object.editmode_toggle()


        if self.ebb_bow_pin_use == True:
            ...
            # select vertex group 1              
            sel_id_A = bpy.context.object.vertex_groups.active_index
            bpy.ops.object.vertex_group_select(sel_id_A)  
         
            bpy.ops.object.editmode_toggle()
            create_vertex_groups_endbow_B(self, context)
            bpy.ops.object.editmode_toggle()
        ...
        if self.ebb_fix_use == True:
            ...
            if self.ebb_bow_pin_use == True:            
                sel_id_B = bpy.context.object.vertex_groups.active_index
                bpy.ops.object.vertex_group_select(sel_id_B)  
        ...
        for vgroup in ob.vertex_groups:
            if self.ebb_bow_pin_use == True:           
                if vgroup.name.startswith("EndBow_B"):
                    ob.vertex_groups.remove(vgroup)
            else:
                if vgroup.name.startswith("EndBow_A"):
                    ob.vertex_groups.remove(vgroup)
