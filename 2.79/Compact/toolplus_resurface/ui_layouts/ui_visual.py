...
def draw_visual_ui(self, context, layout):
            ...
            if context.mode == 'OBJECT': 
                ...
                if context.mode == 'EDIT_MESH':          
                    ...
                    if context.mode == 'SCULPT':                                          
                        ...
                        row = box.row(1)  
                        row.operator("tp_ops.sculpt_shade", text="Flat", icon="MESH_CIRCLE").shade_mode='flat'
                        row.operator("tp_ops.sculpt_shade", text="Smooth", icon="SMOOTH").shade_mode='smooth'  
        ...
        if scene.tp_visual_category == "tp_v6": 
            ...
            if context.mode == 'EDIT_CURVE':
                ...
                row = box.row(1)   
                row.prop(context.object.data, "show_handles", text="Handles:")
                row.operator_menu_enum("curve.handle_type_set", "type", text="")                
                
                row = box.row(1)                 
                row.prop(context.object.data, "show_normal_face", text="Normals:")
                row.operator("curve.switch_direction", text="", icon ="ARROW_LEFTRIGHT")                 
                sub = row.row(1)
                sub.scale_x = 1
                sub.alignment = 'RIGHT'                 
                sub.prop(context.scene.tool_settings, "normal_size", text="S")   
