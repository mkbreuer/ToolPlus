...
def draw_edit_ui(self, context, layout):
        ...
        if scene.types_edit_layout == "tp_e3": 
            ...
            row = box.row(1) 
            row.alignment = 'CENTER'                          
            row.label("Face Fill", icon="OUTLINER_DATA_MESH")
           
            box.separator()            
           
            row = box.row(1)        
            #row.operator("tp_ops.closer", "Quads", icon="LATTICE_DATA").quads = True
            row.operator("mesh.poke", "Poke Mesh")
            row.operator("tp_ops.build_corner", "Build Corner")
          
            row = box.row(1)      
            row.operator("mesh.tris_convert_to_quads",text="Make Quads") 
            row.operator("mesh.quads_convert_to_tris",text="Make Tris")
            ...
            row = box.row(1)                     
            row.operator("mesh.subdivide",text="c1").number_cuts=1
            row.operator("mesh.subdivide",text="c2").number_cuts=2
            row.operator("mesh.subdivide",text="c3").number_cuts=3
            row.operator("mesh.subdivide",text="c4").number_cuts=4
            row.operator("mesh.subdivide",text="c5").number_cuts=5
            row.operator("mesh.subdivide",text="c6").number_cuts=6

            row = box.row(1)                     
            row.operator("mesh.subdivide",text="s1").smoothness=1
            row.operator("mesh.subdivide",text="s2").smoothness=2
            row.operator("mesh.subdivide",text="s3").smoothness=3
            row.operator("mesh.subdivide",text="s4").smoothness=4
            row.operator("mesh.subdivide",text="s5").smoothness=5
            row.operator("mesh.subdivide",text="s6").smoothness=6

            row = box.row(1) 
            row.operator("screen.redo_last", text="Redo Last")  
            ...
            row.label("Knifes", icon_value=button_draw_knife.icon_id)             
