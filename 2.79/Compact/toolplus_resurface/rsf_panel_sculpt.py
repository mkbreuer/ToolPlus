...
class draw_layout_resculpt:
    ...
    def draw(self, context):
        ...
        if display_history == True:
            ...
            row = box.row(1)         
            if mkb_exist() == False:
                row.operator("tp_ops.load_brushes", icon = 'MOD_CAST')
                
            if mkb_exist():
                row.operator("tp_ops.load_brushes", text = 'Remove Brushes', icon = 'MOD_CAST')
            ...
            if tp_props.display_docu:                
                ...
                row = box.row(1)
                row.prop(panel_prefs, 'tab_brush_ui') 
                
                box.separator()  

                row = box.row(1)
                row.prop(panel_prefs, 'tab_brush_quickset', "QuickBrush (!need a reboot!)")   
                ...
                row.prop(panel_prefs, 'tab_sculpt_mask')
                row.prop(panel_prefs, 'tab_snapshot_ui') 
                row.prop(panel_prefs, 'tab_visual_ui')        

                row.prop(panel_prefs, 'tab_sculpt_edit')                
                row.prop(panel_prefs, 'tab_symdim_ui')
                row.prop(panel_prefs, 'tab_modsculpt_ui')               
                row.prop(panel_prefs, 'tab_history_ui')                
                row.prop(panel_prefs, 'tab_custom_sculpt_ui') 
