...
#        bpy.utils.register_class(cls)
    ...
    bpy.utils.register_class(MeshCheckCollectionGroup)

    bpy.types.WindowManager.mesh_check = PointerProperty(type=MeshCheckCollectionGroup)
    if mesh_check_handle:
        bpy.types.SpaceView3D.draw_handler_remove(mesh_check_handle[0], 'WINDOW')
    mesh_check_handle[:] = [bpy.types.SpaceView3D.draw_handler_add(mesh_check_draw_callback,
                            (), 'WINDOW', 'POST_VIEW')]
...
def unregister():
    ...
    bpy.utils.unregister_class(MeshCheckCollectionGroup)
