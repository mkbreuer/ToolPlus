...
class VIEW3D_MT_SnapSet_Menu_Pie(bpy.types.Menu):
    ...
    def draw(self, context):
        ...
        if bpy.context.space_data.pivot_point == 'INDIVIDUAL_ORIGINS':               
            row.operator("tpc_ot.set_pivot", text=" ", icon="ROTATECOLLECTION", emboss = addon_prefs.tpc_use_emposs).tpc_pivot="INDIVIDUAL_ORIGINS"
        else:
        ...
        if bpy.context.space_data.pivot_point == 'MEDIAN_POINT':       
            row.operator("tpc_ot.set_pivot", text=" ", icon="ROTATECENTER", emboss = addon_prefs.tpc_use_emposs).tpc_pivot="MEDIAN_POINT" 
        else:                
        ...
        if bpy.context.space_data.transform_orientation == 'GLOBAL':         
            row.operator("tpc_ot.orient_axis", text="GLO", emboss = addon_prefs.tpc_use_emposs).tpc_axis="GLOBAL"        
        else:        
            row.operator("tpc_ot.orient_axis", text="GLO").tpc_axis="GLOBAL"        

        if bpy.context.space_data.transform_orientation == 'LOCAL':   
            row.operator("tpc_ot.orient_axis", text="LOC", emboss = addon_prefs.tpc_use_emposs).tpc_axis="LOCAL"
        else:
            row.operator("tpc_ot.orient_axis", text="LOC").tpc_axis="LOCAL"

        if bpy.context.space_data.transform_orientation == 'NORMAL':   
            row.operator("tpc_ot.orient_axis", text="NOR", emboss = addon_prefs.tpc_use_emposs).tpc_axis="NORMAL"
        else:    
        ...
        if bpy.context.space_data.transform_orientation == 'GIMBAL':   
            row.operator("tpc_ot.orient_axis", text="GIM", emboss = addon_prefs.tpc_use_emposs).tpc_axis="GIMBAL"
        else:    
            row.operator("tpc_ot.orient_axis", text="GIM").tpc_axis="GIMBAL"                 

        if bpy.context.space_data.transform_orientation == 'VIEW':   
            row.operator("tpc_ot.orient_axis", text="VIW", emboss = addon_prefs.tpc_use_emposs).tpc_axis="VIEW"
        else:    
            row.operator("tpc_ot.orient_axis", text="VIW").tpc_axis="VIEW"    

        if bpy.context.space_data.transform_orientation == 'CURSOR':   
            row.operator("tpc_ot.orient_axis", text="CUR", emboss = addon_prefs.tpc_use_emposs).tpc_axis="CURSOR"
        else:    
