...
def draw_mods_object_edit_layout(self, context, layout):
    ...
    Display_Cast = context.user_preferences.addons[__package__].preferences.tab_cast
    if Display_Cast == True:
        ...
        if is_sdeform == True:
            for mod in [m for m in ob.modifiers if m.type == 'CAST']:   
                row.prop(bpy.context.active_object.modifiers[mod.name], "show_viewport", text="")                          
            row.operator("tp_ops.remove_mods_cast", text="" , icon='PANEL_CLOSE')                                 
            if context.mode == 'EDIT_MESH': 
                row.operator("tp_ops.apply_mods_cast", text="", icon_value=button_apply.icon_id)                                                                                                                                          
            else:
                row.operator("tp_ops.apply_mods_cast", text="", icon_value=button_apply.icon_id)                                                                                                                                          
        else:   
            row.operator("tp_ops.mod_cast", "", icon='DISCLOSURE_TRI_RIGHT')                   

        box.separator()  

        if not tp_props.display_cast: 
            ...
            if obj:
                ...
                for mo in obj.modifiers:
                    ...
                    if mo.type == 'CAST':
                        ...
                        row = box.row(1)  
                        row.label(text="Cast Type:")
                        row.prop(mo, "cast_type", text="")
                        ...
                        sub = row.row(1)     
                        sub.scale_x = 2
                        sub.prop(mo, "factor", text="Factor:")
                        ...
                        sub = row.row(1)     
                        sub.scale_x = 2
                        sub.prop(mo, "radius", text="Radius:")
                        ...
                        sub = row.row(1)     
                        sub.scale_x = 2
                        sub.prop(mo, "size", text="Size:")                      
                      
                        row = box.row(1)  
                        row.prop(mo, "use_radius_as_size")

                        box.separator()  

                        row = box.row()
                        row.label(text="Vertex Group:")
                        row.label(text="Use Object:")                        
    ...
    Display_ModCopy = context.user_preferences.addons[__package__].preferences.tab_modcopy
    if Display_ModCopy == True:
        ...
        if context.mode == 'OBJECT':       
            ...
            row = box.row(1)
            if tp_props.display_copy:            
                row.prop(tp_props, "display_copy", text="", icon="PASTEFLIPDOWN")
            else:
                row.prop(tp_props, "display_copy", text="", icon="PASTEFLIPDOWN")
            ...
            sub = row.row(1)
            sub.scale_x = 0.3                         
            sub.operator("object.copy_selected_modifiers", text=" ", icon="LINENUMBERS_ON")
            sub.operator("tp_ops.to_all", text=" ", icon="LINKED").mode = "modifier, children"
            sub.operator("tp_ops.to_all", text=" ", icon="RESTRICT_SELECT_OFF").mode = "modifier, selected"
            ...
            if not tp_props.display_copy: 
                ...
                row = box.row(1)  
                row.operator("object.copy_selected_modifiers", text="Copy only Selected", icon='LINENUMBERS_ON')
 
                box.separator()  
                                 
                row = box.row(1) 
                button_mods_copy = icons.get("icon_mods_copy") 
                row.label("copy to:", icon_value=button_mods_copy.icon_id) 
                                  
                row = box.row(1)   
                row.operator("tp_ops.to_all", text="Selected").mode = "modifier, selected"
                row.operator("tp_ops.to_all", text="Children").mode = "modifier, children"

                box.separator()                

                row = box.row(1) 
                button_mods_append = icons.get("icon_mods_append") 
                row.label("append to:", icon_value=button_mods_append.icon_id) 
                                  
                row = box.row(1)   
                row.operator("tp_ops.to_all", text="Selected").mode = "modifier, selected, append"
                row.operator("tp_ops.to_all", text="Children").mode = "modifier, children, append"
