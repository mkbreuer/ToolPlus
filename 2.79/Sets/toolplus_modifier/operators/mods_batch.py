...
class VIEW3D_TP_Batch_Modifier(bpy.types.Operator):
    ...
    def draw(self, context):
        ...
        if mod_list:
            ...
            Display_Stack_Remove = context.user_preferences.addons[addon_key].preferences.tab_stack_remove
            Display_Stack_Copy = context.user_preferences.addons[addon_key].preferences.tab_stack_copy

            if Display_Stack_Remove == True or Display_Stack_Copy == True:
                ...
                if obj:
                    ...
                    if mod_list:
                        ...
                        if Display_Stack_Remove == True:

                            row.prop(context.scene, "tp_mods_type", text="")
                            row.operator("tp_ops.mods_by_type", text="Rem. Type")                           

                        if Display_Stack_Copy == True:         
                           
                            if context.mode == 'OBJECT':

                                row = box.row(1)
                                row.operator("tp_ops.to_all", text="2-Childs", icon='LINKED').mode = "modifier, children"    
                                row.operator("tp_ops.to_all", text="2-Selected", icon='RESTRICT_SELECT_OFF').mode = "modifier, selected"
