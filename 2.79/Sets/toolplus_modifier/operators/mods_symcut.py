...
# based on automirror from lapineige  
...
def sym_editmode(self, context):

    addon_key = __package__.split(".")[0]    
    panel_prefs = context.user_preferences.addons[addon_key].preferences 
    self.autosym_edit       = panel_prefs.autosym_edit    
    self.autosym_sculpt     = panel_prefs.autosym_sculpt  

    current_mode = bpy.context.object.mode 

    if self.autosym_edit == True:
...
def sym_apply_mod(self, context):

    addon_key = __package__.split(".")[0]    
    panel_prefs = context.user_preferences.addons[addon_key].preferences   
    self.autosym_apply      = panel_prefs.autosym_apply   

    if self.autosym_apply == True:
        is_mirror = False        
...
class VIEW3D_TP_Negativ_X_symcut(bpy.types.Operator):
    """default: delete negativ X side and add X mirror modifier if enabled"""
    ...
    def execute(self, context):
        ...
        addon_key = __package__.split(".")[0]    
        panel_prefs = context.user_preferences.addons[addon_key].preferences
        self.autosym_symmetrize = panel_prefs.autosym_symmetrize        
        self.autosym_mirror     = panel_prefs.autosym_mirror      
        self.autosym_edit       = panel_prefs.autosym_edit  
        self.autosym_sculpt     = panel_prefs.autosym_sculpt  
       
        if self.autosym_symmetrize == True:
            ...
            sym_editmode(self, context)

            if self.autosym_mirror == True:
            ...
            sym_apply_mod(self, context)

        return {'FINISHED'}


class VIEW3D_TP_Positiv_X_symcut(bpy.types.Operator):
    """delete positiv X side and add X mirror modifier if enabled"""
