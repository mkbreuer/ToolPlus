...
from toolplus_origin.ui_menu_pie     import (VIEW3D_MT_Origin_Menu_Pie)
...
def update_origin_menu(self, context):
    try:
        bpy.utils.unregister_class(VIEW3D_MT_Origin_Menu)
        bpy.utils.unregister_class(VIEW3D_MT_Origin_Menu_Pie)
    ...
    if kc:   
        ...
        if panel_prefs.tab_origin_menu == 'menu':

            bpy.utils.register_class(VIEW3D_MT_Origin_Menu)

            km = kc.keymaps.new(name='3D View', space_type='VIEW_3D')
            kmi = km.keymap_items.new('wm.call_menu', 'D', 'PRESS', ctrl=True) #, shift=True, alt=True)
            kmi.properties.name = "VIEW3D_MT_Origin_Menu"    


        if panel_prefs.tab_origin_menu == 'pie':

            bpy.utils.register_class(VIEW3D_MT_Origin_Menu_Pie)

            km = kc.keymaps.new(name='3D View', space_type='VIEW_3D')
            kmi = km.keymap_items.new('wm.call_menu_pie', 'D', 'PRESS', ctrl=True) #, shift=True, alt=True)
            kmi.properties.name = "VIEW3D_MT_Origin_Menu_Pie"   
            

        if panel_prefs.tab_origin_menu == 'remove':
            ...
            for kmi in km.keymap_items:
                if kmi.idname == 'wm.call_menu':
                    if kmi.properties.name == "VIEW3D_MT_Origin_Menu":
                        km.keymap_items.remove(kmi)
                        break
                 
                    if kmi.properties.name == "VIEW3D_MT_Origin_Menu_Pie":
                        km.keymap_items.remove(kmi)
                        break


# ADD TO HEADER #  
from toolplus_origin.ui_header  import (VIEW3D_HT_Set_Origin_Header_Menu)

def update_origin_header(self, context):

    try:     
        bpy.utils.unregister_class(VIEW3D_HT_Set_Origin_Header_Menu)  
    ...
    if panel_prefs.tab_origin_header == 'prepend':

        bpy.utils.register_class(VIEW3D_HT_Set_Origin_Header_Menu)  

    if panel_prefs.tab_origin_header == 'remove':  
        return None
...
def update_origin_special(self, context):
    ...
    if panel_prefs.tab_origin_special == 'remove':  
        return None
