...
class VIEW3D_OT_BBox_Origin_Set(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        for i in range(self.Back_Left_Top):        
            bpy.ops.tpc.OT_origin_to_bounding_box(mode="cubeback_cornertop_minus_xy")
        
        for i in range(self.Back_Top):   
            bpy.ops.tpc.OT_origin_to_bounding_box(mode="cubeback_edgetop_minus_y")        
   
        for i in range(self.Back_Right_Top):            
            bpy.ops.tpc.OT_origin_to_bounding_box(mode="cubeback_cornertop_plus_xy")             
        #Middle          
        for i in range(self.Back_Left):        
            bpy.ops.tpc.OT_origin_to_bounding_box(mode="cubefront_edgemiddle_minus_x")        
      
        for i in range(self.Back):            
            bpy.ops.tpc.OT_origin_to_bounding_box(mode="cubefront_side_plus_y")        
    
        for i in range(self.Back_Right):            
            bpy.ops.tpc.OT_origin_to_bounding_box(mode="cubefront_edgemiddle_plus_x")         
       
        #Bottom       
        for i in range(self.Back_Left_Bottom):        
            bpy.ops.tpc.OT_origin_to_bounding_box(mode="cubeback_cornerbottom_minus_xy")        
       
        for i in range(self.Back_Bottom):            
            bpy.ops.tpc.OT_origin_to_bounding_box(mode="cubefront_edgebottom_plus_y")       
       
        for i in range(self.Back_Right_Bottom):             
            bpy.ops.tpc.OT_origin_to_bounding_box(mode="cubeback_cornerbottom_plus_xy")                   

        #####

        #Top
        for i in range(self.Middle_Left_Top):
            bpy.ops.tpc.OT_origin_to_bounding_box(mode="cubefront_edgetop_minus_x")

        for i in range(self.Top):
            bpy.ops.tpc.OT_origin_to_bounding_box(mode="cubefront_side_plus_z")

        for i in range(self.Middle_Right_Top):
            bpy.ops.tpc.OT_origin_to_bounding_box(mode="cubefront_edgetop_plus_x")             
         
        #Middle
        for i in range(self.Left):                   
            bpy.ops.tpc.OT_origin_to_bounding_box(mode="cubefront_side_minus_x")
        ...
        for i in range(self.Right):                         
            bpy.ops.tpc.OT_origin_to_bounding_box(mode="cubefront_side_plus_x")
            
    
        #Bottom
        for i in range(self.Middle_Left_Bottom):
            bpy.ops.tpc.OT_origin_to_bounding_box(mode="cubefront_edgebottom_minus_x")

        for i in range(self.Bottom):                     
            bpy.ops.tpc.OT_origin_to_bounding_box(mode="cubefront_side_minus_z")

        for i in range(self.Middle_Right_Bottom):           
            bpy.ops.tpc.OT_origin_to_bounding_box(mode="cubefront_edgebottom_plus_x")

        #####

        #Top                    
        for i in range(self.Front_Left_Top):
            bpy.ops.tpc.OT_origin_to_bounding_box(mode="cubefront_cornertop_minus_xy")

        for i in range(self.Front_Top):
            bpy.ops.tpc.OT_origin_to_bounding_box(mode="cubeback_edgetop_plus_y")

        for i in range(self.Front_Right_Top):
            bpy.ops.tpc.OT_origin_to_bounding_box(mode="cubefront_cornertop_plus_xy")                        

        #Middle                      
        for i in range(self.Front_Left):    
            bpy.ops.tpc.OT_origin_to_bounding_box(mode="cubefront_edgemiddle_minus_y")

        for i in range(self.Front):     
            bpy.ops.tpc.OT_origin_to_bounding_box(mode="cubefront_side_minus_y")

        for i in range(self.Front_Right):    
            bpy.ops.tpc.OT_origin_to_bounding_box(mode="cubefront_edgemiddle_plus_y")

        #Bottom
        for i in range(self.Front_Left_Bottom):         
            bpy.ops.tpc.OT_origin_to_bounding_box(mode="cubefront_cornerbottom_minus_xy")       

        for i in range(self.Front_Bottom):
            bpy.ops.tpc.OT_origin_to_bounding_box(mode="cubefront_edgebottom_minus_y")
        
        for i in range(self.Front_Right_Bottom):
            bpy.ops.tpc.OT_origin_to_bounding_box(mode="cubefront_cornerbottom_plus_xy")             
