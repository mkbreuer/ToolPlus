...
def update_menu(self, context):
    try:
        bpy.utils.unregister_class(VIEW3D_TP_ReCoPlanar_Menu)
    ...
    if context.user_preferences.addons[__package__].preferences.tab_menu_view == 'menu':
        ...
        bpy.utils.register_class(VIEW3D_TP_ReCoPlanar_Menu)
    
        # Keymapping 
        wm = bpy.context.window_manager
        
        km = wm.keyconfigs.addon.keymaps.new(name='3D View', space_type='VIEW_3D')
        kmi = km.keymap_items.new('wm.call_menu', 'X', 'PRESS', ctrl=True, shift=True) #add here your new key event

        #example
        #kmi = km.keymap_items.new('wm.call_menu', 'A', 'PRESS', ctrl=True, alt=True, shift=True) 
       
        kmi.properties.name = "VIEW3D_TP_ReCoPlanar_Menu"
