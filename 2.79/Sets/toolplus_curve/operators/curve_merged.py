...
class VIEW3D_TP_Convert_To_Merged_Mesh(bpy.types.Operator):
    """Convert beveled curve to merged mesh"""
    bl_idname = "tp_ops.convert_to_merged_mesh"
    bl_label = "Convert Merged"
    bl_options = {'REGISTER', 'UNDO'}
    ...
    def execute(self, context):
        ...
        if self.mode == 'UNION' or self.mode == 'SEPARATE':
        ...
        if self.mode == 'MERGE':
            bpy.ops.object.join()
        elif self.mode == 'UNION' and len(selected_objs) > 1:
