...
class BezierSubdivide(bpy.types.Operator):
    """ allows to select multiple segments and split them at custom parameters simultaneously """
...
class BezierIntersection(bpy.types.Operator):
    """ select two segments of to split them at their intersections / cyclic off """
...
class BezierOffset(bpy.types.Operator):
    ...
    bezier = bpy.props.BoolProperty(name="Create",  description=" ", default=False, options={'SKIP_SAVE'})    
...
class VIEW3D_MT_edit_curve_cad(bpy.types.Menu):
    ...
    def draw(self, context):
        ...
        #self.layout.operator(svg_export.SVG_Export.bl_idname, text='Export *.svg')
