...
def settings_load(self):
    tp = bpy.context.window_manager.tp_props_zero
...
class View3D_TP_Zero_to_Global_Axis(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        if self.align_x == True:  
            ...
            if self.tp_switch == "tp_org":                
                ...
                if context.mode == 'OBJECT':
                    bpy.ops.tp_ops.origin_cursor_align(loc_x=True, loc_offset=self.tp_origin_offset)                 
                else:   
        ...
        if self.align_y == True:  
            ...
            if self.tp_switch == "tp_org":
                ...
                if context.mode == 'OBJECT':
                    bpy.ops.tp_ops.origin_cursor_align(loc_y=True, loc_offset=self.tp_origin_offset)                  
                else:   
                    bpy.ops.object.editmode_toggle()
                    bpy.ops.tp_ops.origin_cursor_align(loc_y=True, loc_offset=self.tp_origin_offset)
                    bpy.ops.object.editmode_toggle()
        ...
        if self.align_z == True:  
            ...
            if self.tp_switch == "tp_org":        
                ...
                if context.mode == 'OBJECT':
                    bpy.ops.tp_ops.origin_cursor_align(loc_z=True, loc_offset=self.tp_origin_offset)                    
                else:   
                    bpy.ops.object.editmode_toggle()
                    bpy.ops.tp_ops.origin_cursor_align(loc_z=True, loc_offset=self.tp_origin_offset)  
                    bpy.ops.object.editmode_toggle()
...
class View3D_TP_Zero_to_Global_Axis_Menu(bpy.types.Operator):
    """align origin, object or cursor to global axis"""                 
    bl_idname = "tp_ops.zero_axis_menu"          
    bl_label = "ZeroAxis"                 
    bl_options = {'REGISTER', 'UNDO'}   
    ...
    def execute(self, context):
        ...
        bpy.ops.tp_ops.zero_axis(tp_switch = self.tp_switch, 
...
# "author": "Lell, Anfeo"
class VIEW3D_TP_Origin_Cursor_Align(bpy.types.Operator):
    """align origin to cursor"""
    bl_idname = "tp_ops.origin_cursor_align"
