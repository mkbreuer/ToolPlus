...
def draw_smooth_layout(context, layout):
        ...
        if context.mode == 'EDIT_MESH':          
            ...
            row = box.row(1) 
            row.operator("mesh.mark_sharp", text="Sharp Verts", icon='SNAP_VERTEX').use_verts = True          
            props = row.operator("mesh.mark_sharp", text="", icon='X')
            props.use_verts = True
            props.clear = True
            
            row = box.row(1)  
            row.operator("mesh.mark_sharp", text="Sharp Edges", icon='SNAP_EDGE')
